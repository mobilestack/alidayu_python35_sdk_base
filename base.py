...
def sign(secret, parameters):
    ...
    bytes_parameters = parameters.encode('utf-8')
    # else:
    #     bytes_parameters = parameters.encode('utf-8')

    sign = hashlib.md5(bytes_parameters).hexdigest().upper()
    return sign
...
class RestApi(object):
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        connection = httplib.HTTPConnection(self.__domain, self.__port, timeout, source_address=None)  # updated function
        sys_parameters = {
            P_FORMAT: 'json',
            P_APPKEY: self.__app_key,
            P_SIGN_METHOD: "md5",
            P_VERSION: '2.0',
            P_TIMESTAMP: str(int(time.time() * 1000)),  # was long, now int
        ...
        result = response.read().decode() # added decode(), otherwise, the JSON object must be str, not 'bytes'
        ...
        if 'error_response' in jsonobj:
            ...
            # if jsonobj["error_response"].has_key() :
    ...
    def getApplicationParameters(self):
        ...
        for key, value in self.__dict__.items():  #changed to items, was iteritems
        ...
        for key, value in application_parameter.items():   #changed as well
